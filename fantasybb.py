...
#        nba_schedule = nba_schedule.append(Schedule(team.abbreviation).dataframe,ignore_index=True)
    for team in ['HOU','LAL','POR','DEN','MIN','MIL','WAS','LAC','ORL','DAL',\
    'BOS','PHI','PHO','TOR','MIA','ATL','SAS','UTA','BRK','CHI','SAC','CLE',\
    'OKC','GSW','NOP','IND','MEM','NYK','CHO','DET']:
        print(team)
        nba_schedule = nba_schedule.append(Schedule(team).dataframe,ignore_index=True)
...
def get_players():
    ...
    for page_ind in range(30):
        ...
        for player_ind in range(int(page['count'])):
...
#        "tefirman/FantasySports/master/FantasyBasketball_MissingIDs.csv")
        missingIDs = pd.read_csv("https://raw.githubusercontent.com/" + \
        "tefirman/FantasySports/master/res/basketball/missing_ids.csv")
    except:
...
#        "tefirman/FantasySports/master/FantasyBasketball_MissingIDs.csv",verify=False).text.split('\r')]
        missingIDs = [player.split(',') for player in requests.get("https://raw.githubusercontent.com/" + \
        "tefirman/FantasySports/master/res/basketball/missing_ids.csv",verify=False).text.split('\r')]
...
def get_rates(player_stats,start,as_of,num_sims=1000,reference_games=82):
    ...
    if os.path.exists('FantasyBasketballStats.csv'):
        ...
        if games.SEASON_ID.min()%20000 > start//100:
            ...
            del games['min_week']
#            games = games.loc[games.week <= 20].drop_duplicates().reset_index(drop=True)
...
#        "tefirman/FantasySports/master/FantasyBasketball_NameCorrections.csv")
        corrections = pd.read_csv("https://raw.githubusercontent.com/" + \
        "tefirman/FantasySports/master/res/basketball/name_corrections.csv")
    except:
...
#        "tefirman/FantasySports/master/FantasyBasketball_NameCorrections.csv",verify=False).text.split('\r')]
        corrections = [player.split(',') for player in requests.get("https://raw.githubusercontent.com/" + \
        "tefirman/FantasySports/master/res/basketball/name_corrections.csv",verify=False).text.split('\r')]
    ...
    """ CALCULATE DKFP AND YFP HERE!!! """
    games['YFP'] = 3*games['FG3M'] + 2*games['FG2M'] + games['FTM'] + 1.2*games['REB'] + \
    1.5*games['AST'] + 3*games['STL'] + 3*games['BLK'] - games['TOV']
    games['double_digits'] = (3*games['FG3M'] + 2*games['FG2M'] + games['FTM'] >= 10).astype(int) + \
    (games['REB'] >= 10).astype(int) + (games['BLK'] >= 10).astype(int) + \
    (games['STL'] >= 10).astype(int) + (games['AST'] >= 10).astype(int)
    games['DKFP'] = 3.5*games['FG3M'] + 2*games['FG2M'] + games['FTM'] + 1.25*games['REB'] + \
    1.5*games['AST'] + 2*games['STL'] + 2*games['BLK'] - 0.5*games['TOV'] + \
    1.5*(games['double_digits'] >= 2).astype(int) + 3*(games['double_digits'] >= 3).astype(int)
    del games['double_digits']
    """ CALCULATE DKFP AND YFP HERE!!! """
    
    games.loc[games.MATCHUP.str.contains(' @ '),'home_away'] = 'away'
    games.loc[games.MATCHUP.str.contains(' vs. '),'home_away'] = 'home'
    
    """ Only keeping the specified number of games for each player """
    games = games.groupby(['full_name','home_away']).head(reference_games)
    ...
    for ind in range(player_stats.shape[0]):
        for home_away in ['home','away']:
            home_away_inds = games.home_away == home_away
            if player_stats.loc[ind,'full_name'] in ['PG','SG','G','SF','PF','F','C','Util']:
                inds = games.eligible_positions.apply(lambda x: player_stats.loc[ind,'full_name'] in x) & home_away_inds
            else:
                inds = (games.full_name == player_stats.loc[ind,'full_name']) & home_away_inds
                
            player_stats.loc[ind,'num_games_' + home_away] = inds.sum()
            player_stats.loc[ind,'MIN_avg_' + home_away] = \
            (games.loc[inds,'MIN'].sum() + max(reference_games - inds.sum(),0)*\
            games.loc[home_away_inds,'MIN'].mean())/max(reference_games,inds.sum())
            player_stats.loc[ind,'MIN_std_' + home_away] = \
            ((games.loc[inds,'MIN']**2).sum() + max(reference_games - inds.sum(),0)*\
            (games.loc[home_away_inds,'MIN']**2).mean())/max(reference_games,inds.sum())
            player_stats.loc[ind,'MIN_std_' + home_away] -= \
            player_stats.loc[ind,'MIN_avg_' + home_away]**2
            player_stats.loc[ind,'MIN_std_' + home_away] **= 0.5
            
            """ ADD IN AVERAGES FOR DKFP AND YFP HERE!!! """
            player_stats.loc[ind,'YFP_avg_' + home_away] = \
            (games.loc[inds,'YFP'].sum() + max(reference_games - inds.sum(),0)*\
            games.loc[home_away_inds,'YFP'].mean())/max(reference_games,inds.sum())
            player_stats.loc[ind,'YFP_std_' + home_away] = \
            ((games.loc[inds,'YFP']**2).sum() + max(reference_games - inds.sum(),0)*\
            (games.loc[home_away_inds,'YFP']**2).mean())/max(reference_games,inds.sum())
            player_stats.loc[ind,'YFP_std_' + home_away] -= \
            player_stats.loc[ind,'YFP_avg_' + home_away]**2
            player_stats.loc[ind,'YFP_std_' + home_away] **= 0.5
            player_stats.loc[ind,'DKFP_avg_' + home_away] = \
            (games.loc[inds,'DKFP'].sum() + max(reference_games - inds.sum(),0)*\
            games.loc[home_away_inds,'DKFP'].mean())/max(reference_games,inds.sum())
            player_stats.loc[ind,'DKFP_std_' + home_away] = \
            ((games.loc[inds,'DKFP']**2).sum() + max(reference_games - inds.sum(),0)*\
            (games.loc[home_away_inds,'DKFP']**2).mean())/max(reference_games,inds.sum())
            player_stats.loc[ind,'DKFP_std_' + home_away] -= \
            player_stats.loc[ind,'DKFP_avg_' + home_away]**2
            player_stats.loc[ind,'DKFP_std_' + home_away] **= 0.5
            """ ADD IN AVERAGES FOR DKFP AND YFP HERE!!! """
            
            for stat in ['FTA','FG2A','FG3A','AST','STL','BLK','TOV','REB']:
                player_stats.loc[ind,stat + '_rate_' + home_away] = \
                ((games.loc[inds,stat]/games.loc[inds,'MIN']).sum() + \
                max(reference_games - inds.sum(),0)*(games.loc[home_away_inds,stat]/\
                games.loc[home_away_inds,'MIN']).mean())/max(reference_games,inds.sum())
            for stat in ['FTM','FG2M','FG3M']:
                if games.loc[inds,stat[:-1] + 'A'].sum() > 0:
                    player_stats.loc[ind,stat + '_rate_' + home_away] = \
                    (inds.sum()*games.loc[inds,stat].sum()/games.loc[inds,stat[:-1] + 'A'].sum() + \
                    max(reference_games - inds.sum(),0)*games.loc[home_away_inds,stat].sum()/\
                    games.loc[home_away_inds,stat[:-1] + 'A'].sum())/max(reference_games,inds.sum())
                else:
                    player_stats.loc[ind,stat + '_rate_' + home_away] = \
                    (max(reference_games - inds.sum(),0)*games.loc[home_away_inds,stat].sum()/\
                    games.loc[home_away_inds,stat[:-1] + 'A'].sum())/max(reference_games,inds.sum())
    
    """ COMBINE HOME AND AWAY HERE!!! Do we even need to combine home and away??? """
    player_stats['num_games'] = player_stats['num_games_home'] + player_stats['num_games_away']
    for stat in ['MIN','YFP','DKFP']:
        player_stats[stat + '_avg'] = (player_stats[stat + '_avg_home'] + player_stats[stat + '_avg_away'])/2
        player_stats[stat + '_std'] = (player_stats[stat + '_std_home'] + player_stats[stat + '_std_away'])/2 # Not sure if this is quite right...
    for stat in ['FTA','FG2A','FG3A','AST','STL','BLK','TOV','REB','FTM','FG2M','FG3M']:
        player_stats[stat + '_rate'] = (player_stats[stat + '_rate_home'] + player_stats[stat + '_rate_away'])/2
    """ COMBINE HOME AND AWAY HERE!!! Do we even need to combine home and away??? """
...
def get_schedule():
    ...
    for team in teams.team_key:
        tm = lg.to_team(team)
        for week in range(1,max(19,int(settings['current_week']) + 1)):
            while True:
                try:
...
#    schedule['me'] = (schedule['team_x'] == team_name) | (schedule['team_y'] == team_name)
    
#    if latest_season == 2021:
#        schedule = schedule.append({'week':19,'team_x':'Trust the Process','team_y':"Ashley's Team",\
#        'wins_x':0,'wins_y':0,'FGM_x':0,'FGA_x':0,'FG%_x':0,'FTM_x':0,'FTA_x':0,'FT%_x':0,\
#        'FG3M_x':0,'PTS_x':0,'REB_x':0,'AST_x':0,'STL_x':0,'BLK_x':0,'TOV_x':0,\
#        'FGM_y':0,'FGA_y':0,'FG%_y':0,'FTM_y':0,'FTA_y':0,'FT%_y':0,'FG3M_y':0,\
#        'PTS_y':0,'REB_y':0,'AST_y':0,'STL_y':0,'BLK_y':0,'TOV_y':0},ignore_index=True,sort=False)
#        schedule = schedule.append({'week':19,'team_x':'Suggs my Ball','team_y':"Aaron's Team",\
#        'wins_x':0,'wins_y':0,'FGM_x':0,'FGA_x':0,'FG%_x':0,'FTM_x':0,'FTA_x':0,'FT%_x':0,\
#        'FG3M_x':0,'PTS_x':0,'REB_x':0,'AST_x':0,'STL_x':0,'BLK_x':0,'TOV_x':0,\
#        'FGM_y':0,'FGA_y':0,'FG%_y':0,'FTM_y':0,'FTA_y':0,'FT%_y':0,'FG3M_y':0,\
#        'PTS_y':0,'REB_y':0,'AST_y':0,'STL_y':0,'BLK_y':0,'TOV_y':0},ignore_index=True,sort=False)
...
def player_sim(player,num_sims=1000,week=None):
    ...
    elif week == 18: # All-Star Break combines week 18 and 19
        if (player['nba_team'],18) in nba_schedule.index or (player['nba_team'],19) in nba_schedule.index:
            num_games = num_sims*[nba_schedule.loc[(player['nba_team'],[18,19]),'num_games'].sum()]
        else:
            num_games = num_sims*[0]
    elif week >= 19: # All-Star Break combines week 18 and 19
...
#    if player['full_name'] in ['Jalen Brunson','Will Barton','Dorian Finney-Smith'] and week == 20:
...
def season_sim(schedule,rosters,num_sims=1000,winner=False,current_week=False):
    ...
    team_name = teams.loc[teams.team_key == lg.team_key(),'name'].values[0]
    schedule_sim['me'] = (schedule_sim['team_x'] == team_name) | (schedule_sim['team_y'] == team_name)
...
def possible_sits(rosters,schedule,num_sims=1000):
    ...
    added_value['win_prob' + x_or_y] = added_value['win_prob' + x_or_y].astype(float)
...
def main():
    ...
    parser.add_option('--pickups',action="store",dest="pickups",help="whether to assess possible free agent pickups")
    ...
    parser.add_option('--trades',action="store",dest="trades",help="whether to assess possible trades")
    parser.add_option('--output',action="store",dest="output",help="path of output csv's")
    options,args = parser.parse_args()
    
    if options.name == "Ashley's Team":
        os.rename('oauth2.json','oauth2_Taylor.json')
        os.rename('oauth2_Ashley.json','oauth2.json')
...
#        options.games = 82 # Still working on optimal values
        options.games = 41 # Still working on optimal values
    ...
    if not options.output:
        ...
        if not os.path.exists(options.output + options.name.replace(' ','').replace("'","")):
            os.mkdir(options.output + options.name.replace(' ','').replace("'",""))
        if not os.path.exists(options.output + options.name.replace(' ','').replace("'","") + '/' + str(int(options.as_of)//100)):
            os.mkdir(options.output + options.name.replace(' ','').replace("'","") + '/' + str(int(options.as_of)//100))
        options.output += options.name.replace(' ','').replace("'","") + '/' + str(int(options.as_of)//100)
    if options.output[-1] != '/':
        options.output += '/'
...
#        "tefirman/FantasySports/master/FantasyBasketball_InjuredList.csv")
        injured = pd.read_csv("https://raw.githubusercontent.com/" + \
        "tefirman/FantasySports/master/res/basketball/injured_list.csv")
    except:
...
#        "tefirman/FantasySports/master/FantasyBasketball_InjuredList.csv",verify=False).text.split('\r')]
        injured = [player.split(',') for player in requests.get("https://raw.githubusercontent.com/" + \
        "tefirman/FantasySports/master/res/basketball/injured_list.csv",verify=False).text.split('\r')]
...
#    rosters = rosters.loc[~rosters.full_name.isin(['Spencer Dinwiddie'])]\
#    .append(available.loc[available.full_name.isin(['OG Anunoby'])],ignore_index=True,sort=False)
#    rosters.fantasy_team = rosters.fantasy_team.fillna("Ashley's Team")
#    available = available.loc[~available.full_name.isin(['OG Anunoby'])].reset_index(drop=True)
#    """ HARD CODING ROSTER CHANGES FOR POSSIBLE TRADES!!! """
    ...
    writer = pd.ExcelWriter(options.output + 'FantasyBasketballProjections_{}Week{}.xlsx'\
    ...
    if options.subs:
        ...
        writer.sheets['Subs'].freeze_panes(1,2)
    ...
    if options.name == "Ashley's Team":
        os.rename('oauth2.json','oauth2_Ashley.json')
        os.rename('oauth2_Taylor.json','oauth2.json')
