...
def get_schedule(season: int, playoffs=False):
    ...
    if not playoffs:
        schedule = schedule.loc[schedule.week_num.str.isnumeric()].reset_index(
            drop=True
        )
...
def get_boxscore(game_id: str):
    ...
    game_stats = pd.concat(
        [
            parse_table(raw_text, "player_offense"),
    ...
    starters = pd.concat(
        [parse_table(raw_text, "home_starters"), parse_table(raw_text, "vis_starters")]
    )
    snaps = pd.concat(
        [
            parse_table(raw_text, "home_snap_counts"),
            parse_table(raw_text, "vis_snap_counts"),
        ]
    )
    snaps = (
        snaps.loc[~snaps.player_id.isin(starters.player_id.tolist())]
        .sort_values(by=["off_pct", "def_pct", "st_pct"], ascending=False)
        .reset_index(drop=True)
    )
    starters = pd.merge(
        left=pd.concat([starters.iloc[::-1], snaps]),
        right=game_stats[["player", "player_id", "team"]],
        how="inner",
        on=["player", "player_id"],
    )
    starters["dummy"] = 1
    ...
    game_stats = pd.merge(
        left=game_stats,
        right=starters[["player", "player_id", "team", "pos", "string"]],
        how="inner",
    ...
    for season in range(start // 100, finish // 100 + 1):
        ...
        if season == start // 100:
            season_sched = season_sched.loc[
                season_sched.week_num >= start % 100
            ].reset_index(drop=True)
        if season == finish // 100:
            season_sched = season_sched.loc[
                season_sched.week_num <= finish % 100
            ].reset_index(drop=True)
        for ind in range(season_sched.shape[0]):
            print(season_sched.loc[ind, "boxscore_word"])
            stats = pd.concat(
                [stats, get_boxscore(season_sched.loc[ind, "boxscore_word"])]
            )
    return stats
