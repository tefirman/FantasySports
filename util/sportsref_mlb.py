...
def get_schedule(season: int, playoffs=False):
    ...
    season_id = raw_text.find(attrs={'data-label':"MLB Schedule"}).attrs['id'].split('_')[0]
    games = raw_text.find(id="div_" + season_id).find_all('p',attrs={'class':'game'})
    schedule = pd.DataFrame()
    for game in games:
        links = game.find_all('a')
        entry = {'away_team':links[0].text,
    ...
    if playoffs:
        season_id = raw_text.find(attrs={'data-label':"Postseason Schedule"}).attrs['id'].split('_')[0]
        games = raw_text.find(id="div_" + season_id).find_all('p',attrs={'class':'game'})
        for game in games:
            links = game.find_all('a')
            entry = {'away_team':links[0].text,
...
def get_boxscore(game_id: str):
    ...
    date = pd.to_datetime(description.split(' Box Score: ')[-1].split(' | ')[0],infer_datetime_format=True)
    batting = pd.concat([parse_table(raw_text,away.replace(' ','').replace('.','') + 'batting'),
                         parse_table(raw_text,home.replace(' ','').replace('.','') + 'batting')])
    batting = batting.loc[~batting.player.str.endswith(' P') & ~batting.player.isin(['Team Totals'])]
    ...
    pitching.loc[pitching.player.str.contains(pat=', W \(\d{1,2}-\d{1,2}\)',regex=True),'W'] = 1.0
    pitching.loc[pitching.player.str.contains(pat=', L \(\d{1,2}-\d{1,2}\)',regex=True),'L'] = 1.0
    pitching.loc[pitching.player.str.contains(pat=', H \(\d{1,2}\)',regex=True),'HD'] = 1.0
    pitching.loc[pitching.player.str.contains(pat=', S \(\d{1,2}\)',regex=True),'SV'] = 1.0
...
def get_games(start: str, finish: str):
    ...
    for season in range(start.year,finish.year + 1):
        ...
        if season == start.year:
            season_sched = season_sched.loc[season_sched.date >= start].reset_index(drop=True)
        if season == finish.year:
            season_sched = season_sched.loc[season_sched.date <= finish].reset_index(drop=True)
